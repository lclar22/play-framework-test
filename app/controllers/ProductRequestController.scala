...
/*
  def request = Action {
    val insumosNames = getVeterinarioNamesMap()
    val proveeNames = getProveeNamesMap()
    Ok(views.html.productRequest_request(newForm, insumosNames, proveeNames))
  }

  def request_row = Action {
    val insumosNames = getVeterinarioNamesMap()
    val proveeNames = getProveeNamesMap()
    Ok(views.html.productRequest_request_row(newForm, insumosNames, proveeNames))
  }
*/
...
/*
  def add_request_row = Action.async { implicit request =>
    newForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.productRequest_request(errorForm, Map[String, String](), Map[String, String]())))
      },
      res => {
        repo.create(res.productId, res.proveedorId, res.amount, res.amountLeft).map { _ =>
          Redirect(routes.ProductRequestController.request)
        }
      }
    )
  }
