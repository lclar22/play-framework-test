$ ->
  $.get "/users", (rows) ->
    $.each rows, (index, row) ->
      ...
      links = $("<td>").html '<a href="/user_update/' + row.id + '" class="btn btn-primary">Editar</a>' + '<a href="/user_remove/' + row.id + '" class="btn btn-danger">Eliminar</a>' + '<a href="/user_show/' + row.id + '" class="btn btn-info">Mostrar</a>' + '<a href="/' + row.type_1 + '_profile/' + row.id + '" class="btn btn-info">Profile</a>'
